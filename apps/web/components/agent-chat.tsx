...
export function AgentChat({ chatId, currentAgent }: AgentChatProps) {
  ...
  // Check streaming support on mount
  useEffect(() => {
    const checkStreamingSupport = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setStreamingSupported(config.streamingSupported);
        }
      } catch (error) {
        console.error('Failed to check streaming support:', error);
    ...
    try {
      ...
      const response = await fetch('/api/agent', {
        ...
        body: JSON.stringify({
          ...
          stream: streamingSupported, // Use streaming only if supported
      ...
      // Handle response based on streaming support
      if (streamingSupported) {
        ...
        setToolStatus(''); // Clear any tool status
        ...
        await addMessage({
          ...
          role: 'assistant',
          content: data.output,
          agentName: data.agent || 'AI Assistant',
        });
      }
    ...
    <div className="flex flex-col h-full w-full items-center">
      <div className="flex flex-col h-full max-w-2xl w-full relative">
        <div ref={scrollRef} className="flex-1 p-4 pb-32 overflow-y-auto">
          ...
          {chatId && messages.length === 0 && currentAgent && (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              {/* Agent Avatar - Large with personal touches */}
              <div className="mb-6 relative">
                <div className="absolute -inset-4 bg-gradient-to-r from-primary/20 via-purple-500/20 to-pink-500/20 rounded-full blur-xl"></div>
                {currentAgent.avatarUrl ? (
                  <Avatar className="w-20 h-20 relative z-10 floating-animation">
                    <AvatarImage
              ...
              {/* Agent Info with personal styling */}
        ...
        <form
          ...
          className="fixed bottom-0 p-3 md:p-4 bg-background/80 backdrop-blur-md border-t border-primary/10 transition-[left] shadow-lg"
          style={{
            left: isMobile
              ? 0
        ...
        >
          ...
          <div className="flex gap-1.5 md:gap-2">
            <Button
              ...
              className="h-9 w-9 md:h-10 md:w-10 flex-shrink-0 hover:bg-primary/10 hover:border-primary/50 transition-all"
            >
            ...
            <Input
              ...
              className="flex-1 h-9 md:h-10 text-sm md:text-base focus:ring-2 focus:ring-primary/20 transition-all"
            />
            ...
            <Button
              ...
              className="h-9 w-9 md:h-10 md:w-10 flex-shrink-0 bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 hover:to-purple-600/90 transition-all pulse-glow relative z-50"
            >
